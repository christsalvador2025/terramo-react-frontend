...
const StakeholderAnalysisDashboardESG = () => {
  ...
  const [selectedGroupsShowInTable, setSelectedGroupsShowInTable] = useState(new Set<number | string>());
  ...
  React.useEffect(() => {
    if (stakeholderData?.stakeholder_groups_data_plot) {
      ...
      stakeholderData.stakeholder_groups_data_plot.forEach((group: any) => {
        if (group.is_global) defaultGroups.add(group.id);
      });
  ...
  React.useEffect(() => {
    if (stakeholderData?.stakeholder_groups_data_plot) {
      const defaultGroupsInTable = new Set<number | string>();
      stakeholderData.stakeholder_groups_data_plot.forEach((group: any) => {
        // group.is_global ||  
        if (group.show_in_table && group.has_responses && !group.is_global) defaultGroupsInTable.add(group.id);
      });
  ...
  const isGroupChecked = (group: any) =>
    // group ? if in - selectedGroupsShowInTable = is only 
    // group.is_default || (!isGroupToggleDisabled(group) && selectedGroups.has(group.id)); origginal
    group.is_global || (!isGroupToggleDisabled(group) && selectedGroups.has(group.id));

  // for group toggleshow group in table
  const isGroupShowInTable = (group: any) => {
    ...
    //   return newSet;
    // });
    if (group.is_global || (selectedGroupsShowInTable.has(group.id))){
      return true;
    }  
